...
helm unittest universal-chart/ --strict --file 'tests/singular_blocks_test.yaml'
...
**Done = all true**: `helm lint --strict` passes, kubeconform schema validation passes (see schema validation command above), `helm unittest --strict` passes,
`helm template test universal-chart/ -f universal-chart/ci/test-values.yaml` succeeds (values validate against `values.schema.json`),
...
- **Singular blocks or plural dicts**: Core resources — `deployment`, `statefulset`, `daemonset`, `job`, `cronJob`, `service`, `hpa`, `pdb`, `pvc`, `externalSecret`, `imageUpdater` — are singular (at most one per release). Multi-instance resources use dict keys (`configMaps`, `secrets`, `certificates`, `clusterIssuers`, `issuers`, `clusterExternalSecrets`, `secretStores`, `clusterSecretStores`, `istioVirtualServices`, `istioGateways`, `istioDestinationRules`, `istioAuthorizationPolicies`, `istioPeerAuthentications`, `httpRoutes`, `persistentVolumes`, `storageClasses`, `serviceMonitors`). `serviceAccount` is a list. `defaults:` replaces the removed `*General` sections.
- **`disabled: true`** on dict-based resources (certificates, issuers, clusterIssuers, clusterExternalSecrets, secretStores, clusterSecretStores, istio*, httpRoutes, persistentVolumes, storageClasses, serviceMonitors) suppresses rendering — keep it inside the `range` after the guard so `---` doesn't leak. `configMaps` and `secrets` do **not** support `disabled` — omit them from the dict entirely to skip creation. Singular blocks have no `disabled` — omit the block entirely to skip creation.
...
- **Defaults** (`defaults:`) provide shared configuration for all resources. Each workload block references `defaults` fields directly (no merge cascade), except: (a) two security contexts which are deep-merged — `containerSecurityContext` three ways (`defaults.containerSecurityContext` → workload-level → per-container override, via `mustMergeOverwrite` in `_container.tpl`); and `podSecurityContext` two ways (`defaults.podSecurityContext` → workload-level, via `mustMergeOverwrite` in `_pod.tpl`; setting an empty dict `{}` at workload level clears the cascade entirely); and (b) three additive list fields — `extraImagePullSecrets`, `extraVolumes`, and `extraVolumeMounts` are appended (both defaults and workload-level values are included). CRD passthrough resources have NO defaults — `spec:` is rendered verbatim via `helpers.tplvalues.render`.
...
- **`defaults.resources`** — when set (no default value; was intentionally removed), the value cascades to every container that does not define its own `resources:` block. Useful for setting cluster-wide request/limit defaults without touching every workload. Code path: `_container.tpl` applies `defaults.resources` when the container-level `resources:` key is absent.

## Template Authoring

### Singular block skeleton

```yaml
{{- if .Values.<key> }}
  {{- $v := .Values.<key> }}
  {{- $name := $v.name | default $.Release.Name | trunc 63 | trimSuffix "-" }}
---
...
### Dict-based resource skeleton (range + disabled guard)

```yaml
{{- range $name, $val := .Values.<pluralKind> }}
  {{- if not ($val.disabled | default false) }}
---
apiVersion: ...
kind: <Kind>
metadata:
  name: {{ $val.name | default (include "helpers.app.fullname" (dict "name" $name "context" $)) | trunc 63 | trimSuffix "-" }}
  ...
  {{- end }}
{{- end }}
```
...
- User-supplied string and map values should be rendered via `helpers.tplvalues.render` to allow Go-template expressions in values (e.g. `.Release.Name` references).
- CRD passthrough resources render `spec` verbatim: `{{- include "helpers.tplvalues.render" (dict "value" $val.spec "context" $) | nindent 2 }}`. Do not invent abstraction layers over CRD `spec:` blocks.
...
| `_pod.tpl` | `helpers.pod` — the single pod spec used by **every** workload (~200 lines). Changes here ripple to deployments, statefulsets, daemonsets, jobs, and cronjobs — test all five. |
...
tests:
  - it: <lowercase sentence>
    set:
      <singularKind>.<field>: <value>         # singular blocks
      <pluralKind>.<instance>.<field>:        # dict-based resources
    asserts:
      - <assertion>
```
...
- Do not wrap CRD `spec:` blocks in custom abstractions — use `helpers.tplvalues.render` passthrough.
...
- Do not treat anything under `examples/` (e.g. `minimal-values.yaml`, `diag-values.yaml`) as chart input — they are scratch/illustrative.
