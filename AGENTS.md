...
- **Singular blocks or plural dicts**: Core resources — `deployment`, `statefulset`, `daemonset`, `job`, `cronJob`, `service`, `hpa`, `pdb`, `pvc`, `imageUpdater` — are singular (at most one per release). Multi-instance resources use dict keys (`configMaps`, `secrets`, `certificates`, `clusterIssuers`, `issuers`, `externalSecrets`, `clusterExternalSecrets`, `secretStores`, `clusterSecretStores`, `istioVirtualServices`, `istioGateways`, `istioDestinationRules`, `istioAuthorizationPolicies`, `istioPeerAuthentications`, `httpRoutes`, `persistentVolumes`, `storageClasses`, `serviceMonitors`). `serviceAccount` is a list.
- **`disabled: true`** on dict-based resources (certificates, issuers, clusterIssuers, externalSecrets, clusterExternalSecrets, secretStores, clusterSecretStores, istio*, httpRoutes, persistentVolumes, storageClasses, serviceMonitors) suppresses rendering — keep it inside the `range` after the guard so `---` doesn't leak. `configMaps` and `secrets` do **not** support `disabled` — omit them from the dict entirely to skip creation. Singular blocks have no `disabled` — omit the block entirely to skip creation.
