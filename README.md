...
For most apps, two tiers is enough — set shared values in `defaults:` and per-workload values directly on the instance. The middle tier `*General:` exists for a specific niche case covered in **3-Tier Defaults Cascade** below.
...
- **You have ≤2 instances of that kind.** The cost of an extra tier (readers must mentally merge three places to know what renders) only pays off when there are enough instances for DRY savings to dominate. With 1-2 of a kind, just put it on the instance.
...
Set `startupProbe`, `livenessProbe`, `readinessProbe` directly on the container. Mix and match — set only the ones you need. The moment **any** of the three is set, shorthand generation is skipped entirely and only what you've defined is rendered, so set all three explicitly if you want all three.
